...
The [serverless driver from Planetscale](https://planetscale.com/blog/introducing-the-planetscale-serverless-driver-for-javascript) is super important,
...
- **GPT-3.5-turbo**: The biggest problem I had with gpt-3.5-turbo is that it would make a drink with literally anything, no matter
  ...
  aren't possible.
- **Llama-2-70b-chat**: I tried this one out through Replicate. Llama-2-70b solved my problem of creating drinks with weird ingredients, but I could
...
- **GPT-4**: This got me the closest. Gpt-4 mostly respected my rules, and would return a token "IMPOSSIBLE" when it decided the
  ...
  irish cream, vodka, cinnamon, and espresso, it would tell me to grind the beans, brew a cup of coffee, and add cinnamon. Not quite what I was going for:
...
I settled on gpt-4, which at the time of writing is still what Thirsty Bot uses. It's a bit more expensive, but it seemed best suited for my task. With a lot of iteration on my prompt with the
...
I'm still working on tuning how I direct DALL·E 3. My prompt right now includes the steps to make the drink, as well as the name
...
> It's well-documented that [I don't like useEffect](https://keegan.codes/blog/reacting-to-prop-changes-in-react-functional-components) and try to avoid it entirely.
...
The button itself is a client component that uses the `useFormState` hook in order to show the disabled state while it's pending:.
...
Everything else is pretty by the books for Next.js! Styling is all done with Tailwind (and a small amount of CSS for animations)
...
The application runs on Vercel, and generated images are served out of Cloudflare R2.
