...
export const title = 'Building an AI Web Application in 72 Hours';
...
hours as part of a
litle personal "hackathon", and I learned a lot in the process. In this post, I'll share some of what I
learned, and do
...
### Day 2 - Mobile Support & Polish; Final Features
...
Finally, I continue to take feedback on my list of ingredients, adding more as people remind me of their
...
My database is [Planetscale](https://planetscale.com/) on the smallest plan, `PS-10`. I use Planetscale
for this site (keegan.codes), so it made
...
is important,
because it means I can execute queries using the fetch API and an HTTP connection. Connecting to the database
...
> a ton of data on a branch instead of the production instance.
...
- **GPT-3.5-turbo**: The biggest problem I had with gpt-3.5-turbo is that it would make a drink
  ...
  aren't possible.
- **Llama-2-70b-chat**: I tried this one out through Replicate. Llama-2-70b solved my problem of
...
- **GPT-4**: This got me the closest. Gpt-4 mostly respected my rules, and would return a token
  "IMPOSSIBLE" when it decided the
  ingredients couldn't go together. It also only vary rarely added ingredients. Sometimes it would add simple
  syrup when it wasn't included, but that
...
I tried a few others as well, but they all had some of the issues presented here. In the end, I narrowed it down
...
> something I'm not familiar with yet. I'm hoping to look into this down the road
> to save on cost, and make things faster.
...
> returns drink recipes. Regardless, that's an improvement for another day.
...
For the images to go with each recipe, I used DALL·E 3. I've been using its predecessor
...
in the recipe this way, however it's difficult to make DALL·E 3 leave off text, and
...
Thirrsy Bot has a form to redeem vouchers for free credits. This way, I can give them away to friends and family
who are testing it for me, and also on social media and product hunt, for example:

![](thirsty-bot-example:redeem.png)

This uses server actions to sumit your code, validate it, and give you the credits! No need for any client-side logic!
...
The API is also built with Next.js. I have APIs for the builder and fetching options, generating cocktails,
and webhooks for handling Stripe billing events and Clerk user management events. I like building with Next.js API
...
export const POST = async (req: NextRequest) => {
  ...
  // Get the body as a string. This wasn't intuitive and was kind of odd.
  // Doing req.json() got the event payload, but because of the Next.js body parser,
  // the stripe SDK couldn't parse it and verify the signature correcly.
...
I looked at a number of options for authentication. Some people recommended like next-auth, others like Supabase auth.
In the end, I went with Clerk. The integration was super simple, and I was able to get login working within a matter of
minutes, not hours or days. It's an almost entirely no-code solution, with just a few default UI components,
route handlers, and edge middleware. I also have a couple webhook handlers that can react to Clerk's user events,
like user creations to add free credits to each new account.
...
from this website, which
runs on edge functions. Overall I haven't noticed too many issues with cold starts, although it's definitely
more noticeable compared to the things I run on the edge.
...
1. Prompt engineering is an art! I read a lot of advice and had to iterate a lot when prompting GPT-4 in order
   to get reliable results I was happy with.
