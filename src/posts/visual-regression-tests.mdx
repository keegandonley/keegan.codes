...
export const published = new Date(2023, 9, 17);
export const shortCodes = ["regression-testing"];
...
I've [written about testing before](https://keegan.codes/blog/common-types-of-web-application-tests-and-how-they-can-improve-performance-and-reliability) including visual tests, but here's a quick recap:

Rather than test that a component behaves correctly, or has the right markup, a visual test actually loads
...
Here, I have just one story stories: `Primary`. We can easily add a few more:
...
I set up my visual regression tests using [Jest](https://jestjs.io/) and [jest-image-snapshot](https://github.com/americanexpress/jest-image-snapshot) from
American Express. Here's how it works: the test framework saves a reference image on the first run, and then
...
In this case, my story names will be parsed as `Primary` and `Empty` and `Overflow` - that’s a good
start, but we need to convert them to story IDs. My story files are using the [component
...
is [here](https://github.com/ComponentDriven/csf/blob/4c735fea4f0c9605b93497238303cb4ab9304727/src/toStartCaseStr.ts#L4)
on Github if you want to check it out, but I included a modified version below.
...
You can read more about [how GitHub handles Git LFS](https://docs.github.com/en/repositories/working-with-files/managing-large-files/configuring-git-large-file-storage), which is what I use.
...
fast, so I run them on every pull request to make sure I can deploy with confidence!
