...
needs a unique key. This is so React can keep track of which components have changed, and which ones need to be re-rendered.

We can render the file tree with the following code, passing in the data object build earlier:
...
The first page of the blog list is rendered server-side,
and additional pages of data are fetched and rendered client-side. Additional
pages are recursively rendered, only after the previous page has been finished.

It's a lot more complex than what I'll show in the examples, but you can check out
...
and the next page was recursively rendered.
...
and how it can be used conditionally based on children:
...
Let's revisit the rocket countdown example, but in this situation we want to count down by 2:
...
const RocketLaunch = (props) => {
  ...
  return (
    <>
      {props.count}...
      <RocketLaunch count={props.count - 2} />
    </>
  );
};
...
In this example, we'll count down by 2 just fine: `10`, `8`, `6`, `4`, `2`, `liftoff!`. But what if we change the initial count to 9?
...
This issue can be particularly tricky, because the base case may be fine in most situations,
but when taking in user input, it might fail! Always be sure to think carefully about the base case
when using recursion.

> Here's the same interactive example, but using the new base-case and a countdown interval of 2:
...
container, but that would have introduced some complexity and many additional elements that
aren't necessary.

The same holds true for our countdown example. It might be fine to create a new element wrapping each number
when we only have 10, but if this takes user input and doesn't have a sane maximum, what would happen if a user
...
> If you want to read more about browser performance and maximum DOM depth, the [Chrome for Developers](https://developer.chrome.com/en/docs/lighthouse/performance/dom-size/) article is a great resource.

## Final Thoughts

Recursion is a powerful tool in programming, and it can be used in React to simplify component rendering.
It can also be used incorrectly to greatly _increase_ complexity and cause performance issues. Always be sure that you're using the right
