...
In order to keep my blog's performance high, I want all the data fetching to happen on the server side when the page is generated. I leverage NextJS's `getStaticProps` for this.
...
const res = ReactDOMServer.renderToString(
  <div className="border-gray-200 border-1 rounded-lg shadow max-w-[500px] mx-auto p-4 mb-12 bg-white dark:bg-gray-800 text-black dark:text-white">
    <div className="flex">
    ...
    <div className="mt-4 w-full flex !text-blue-400">
      <a
        href={tweet}
        target="_blank"
        rel="noopener noreferrer"
        className="flex-grow w-full border border-blue-400 rounded-full text-center py-1 !text-blue-400 hover:bg-blue-400 hover:!text-white transition-all no-underline cursor-pointer twitter"
      >
