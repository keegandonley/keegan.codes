...
export const setHasChosenThemeCookie = () => {
  ...
  document.cookie = `chosen-theme=true; path=/; domain=${cookieDomain}; expires=Tue, 19 Jan 2038 04:14:07 GMT`;
};

export const setThemeCookie = (theme: Theme) => {
  const cookieDomain =
    process.env.NODE_ENV === "development" ? "localhost" : "keegan.codes";
  document.cookie = `theme=${theme}; path=/; domain=${cookieDomain}; expires=Tue, 19 Jan 2038 04:14:07 GMT`;
};
